...
> ⚠️ **入出金履歴に初期入金が含まれている場合は予算を `0` に設定してください。**
> `input/sbi/` に入出金振替履歴CSV（入金データ）が含まれている場合、その入金額は自動的に予算として取り込まれます。この場合に別途予算を設定すると二重計上になります。`project.yaml` で `budget: 0` を設定するか、`prepare` 実行時に予算を `0` と入力してください。
